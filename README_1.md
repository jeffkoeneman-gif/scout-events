...
Each published event is written to the Drive folder **Scout — Event Links** as
`scout-event-<code>.json`, and the script serves it by code. So the link is just:

```
https://attend.scout-financial.com/?c=evt1a1nz9104
```

Unpublishing an event removes that file, and the link stops working the same
moment. Reissuing writes a new one and bins the old. Nothing to do by hand.

Older long links (`?e=...`) still work, so nothing printed before this change
is stranded.

### Pointing it at your own domain

1. In DNS for scout-financial.com, add a **CNAME**: name `attend`, value
   `jeffkoeneman-gif.github.io`
2. GitHub repo → **Settings → Pages → Custom domain** → `attend.scout-financial.com` → Save
3. Wait for the green check, then tick **Enforce HTTPS**
4. Portal → **Settings → Public site URL** → `https://attend.scout-financial.com`

Every link and QR code made after that uses the new address. Republish any event
whose printed QR code you want to reissue.
